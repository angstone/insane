...
test("throws validation error when such option is provided", function (t) {
  try {
    insane('<script>"foo"</script>', { throwAsValidationError: true });
...
test("should block some out of div", function (t) {
  try {
    insane("</div>maculated text<div>", {
      allowedTags: [],
...
test("should accept normal text with no html", function (t) {
  t.equal(
    insane("joaquin Marcosd sdf%$%* dfasdf921 -- @#$#@$", { allowedTags: [] }),
    "joaquin Marcosd sdf%$%* dfasdf921 -- @#$#@$"
  );
  t.end();
});

test("should accept basic markdown without html", function (t) {
  t.equal(
    insane(`# Tomates **verdes** são bem melhores", { allowedTags: [] })
*italicized text*
...
3. Third item
`),
    `# Tomates **verdes** são bem melhores", { allowedTags: [] })
*italicized text*
